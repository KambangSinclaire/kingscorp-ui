...
<div class="bg-white">
  ...
  <section class="p-4">
    <section data-section-id="3" data-category="ta-headers" data-component-id="52511d2d_02_awz" class="py-4 px-4 mb-4 rounded-md" style="background:rgb(227, 243, 240);">
              <div class="flex flex-wrap -mx-3 items-center">
                <div class="w-full lg:w-1/2 flex items-center mb-5 lg:mb-0 px-3">
                  <span class="inline-flex justify-center items-center w-16 h-16 mr-4 bg-blue-500 rounded">
                ...
                <div class="w-full lg:w-auto ml-auto px-3">
                  <a class="inline-flex items-center py-2 px-3 mr-3 text-xs text-blue-500 font-medium bg-indigo-50 hover:bg-indigo-100 rounded">
  ...
  mounted(){
    ...
    for(let [key,value] of Object.entries(AppActionEvents)){
      if(key != 'user' && key != 'role' && key != 'permission' && key != 'group' && key != 'preference' && key!="client"  && key!="inventory"  && key!="credit" && key!="notification"   ){
        (AppActionEvents as any)[`${key}`]?.retrieve !== undefined && this.$store.dispatch((AppActionEvents as any)[`${key}`]?.retrieve)
        emptyArray.push({
            title: key,
            range: 10,
            btnMore: 78,
